...
static void BM_Utils_DomainValidation(benchmark::State &state)
{
  std::vector<std::string> domains = {
      "example.com", "www.google.com", "sub.domain.test.org",
      "invalid..domain", "toolong" + std::string(250, 'a') + ".com",
      "valid-domain.net", "123.456.789.012"};
...
{
  std::vector<std::string> domains = {
      "Example.COM", "WWW.GOOGLE.COM", "Test.Domain.ORG",
...
static void BM_CacheUtils_KeyGeneration(benchmark::State &state)
{
  ...
  for (auto _ : state)
  {
    auto key = cache_utils::generate_cache_key(
        domains[index % domains.size()], RecordType::A, RecordClass::IN);
